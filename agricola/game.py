...
class AgricolaGame(EventGenerator):
  ...
  def start_round(self):
    ...
    self.add_steps(list(reversed(roundstart_stepstack)))
    self.is_start_of_round = False
    return itertools.cycle(new_order)

  def start_turn(self, player_idx):
      player = self.players[player_idx]
      startturn_stepstack = [ResourceTradingStep(player), ActionSelectionStep(player)]
      self.add_steps(list(reversed(startturn_stepstack)))
  ...
  def end_round(self, player_order):
    ...
    self.is_start_of_round = True
    self.add_steps(list(reversed(end_round_steps)))
  ...
  def end_stage(self, player_order):
    ...
    self.add_steps(list(reversed(end_stage_steps)))
    self.is_end_of_stage = False
